...
/* ============================================================
   ...
   (versione deduplicata per Next.js App Router)
   ============================================================ */
...
function findEmptyCell(board: Board) {
  for (let r = 0; r < 9; r++)
    for (let c = 0; c < 9; c++)
      if (board[r][c] === 0) return [r, c] as const;
  return null;
}
...
function generateFullSolution(rng: () => number = Math.random) {
  ...
  for (let box = 0; box < 9; box += 4) {
    ...
    let k = 0;
    for (let r = br; r < br + 3; r++)
      for (let c = bc; c < bc + 3; c++) board[r][c] = nums[k++];
  }
  return solveBoard(board)!;
}
function countSolutions(board: Board) {
  return solveBoard(board, true, 2) as number;
}
function digHolesFromSolved(
  solved: Board,
  difficulty: Diff,
...
function serializeNotes(notes: Set<number>[][]) {
  return notes.map((row) => row.map((s) => Array.from(s)));
}
function deserializeNotes(raw: number[][][]) {
...
const pastel = {
  bg: "bg-gradient-to-br from-zen-mint to-zen-cream",
  card: "backdrop-blur-xl bg-white/70 border border-zen-sage/60 rounded-2xl",
  given: "text-zen-forest",
  entry: "text-zen-forest",
  conflict: "text-red-600 bg-red-50",
  selected: "ring-2 ring-zen-sage",
  keypad: "bg-zen-mint hover:bg-zen-mint",
} as const;

function cx(...xs: (string | undefined | false)[]) {
...
function Btn(props: {
  ...
  const { children, onClick, disabled, variant = "outline", className = "" } =
    props;
  const base =
  ...
  const styles = {
    primary: "bg-zen-sage text-zen-forest hover:bg-zen-grid",
    secondary: "bg-zen-cream text-zen-forest hover:bg-zen-grid",
    outline: "border border-zen-sage text-zen-forest hover:bg-zen-mint",
    ghost: "text-zen-forest hover:bg-zen-mint",
  } as const;
  return (
    <button
      onClick={onClick}
...
function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3">
      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-[92vw] max-w-md p-5 border border-zen-sage">
        <div className="flex items-start gap-3">
          {showIcon && <Chick className="shrink-0 w-10 h-10" />}
          <div className="min-w-0">
            <h3 className="text-lg font-semibold text-zen-forest">{title}</h3>
            {description && (
              <div className="text-sm text-zen-forest mt-1">{description}</div>
            )}
            {children}
          </div>
        </div>
      ...
      {Array.from({ length: total }).map((_, i) => (
        <span
          ...
          className={cx(
            ...
            i === index ? "bg-zen-mint" : "bg-zen-grid"
          )}
        />
      ))}
    </div>
...
============================================================ */
export default function ZennySudoku() {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      setApp((s: any) => {
        ...
        const now = Date.now();
        const base = s.timer.startedAt
          ? Math.floor((now - s.timer.startedAt) / 1000)
          : 0;
  ...
  function getMergedBoard(entriesOverride?: Board) {
    ...
    return Array.from({ length: 9 }, (_, r) =>
      Array.from({ length: 9 }, (_, c) => app.puzzle![r][c] || e[r][c] || 0)
    );
  }
  function cellConflicts(r: number, c: number, entriesOverride?: Board) {
    ...
    for (let rr = br; rr < br + 3; rr++)
      for (let cc = bc; cc < bc + 3; cc++)
        if ((rr !== r || cc !== c) && b[rr][cc] === v) return true;
    return false;
  }
  function setSelected(r: number, c: number) {
    setApp((s: any) => ({ ...s, selected: { r, c } }));
  }
  ...
  function pushHistory(prev: any) {
    setApp((s: any) => ({ ...s, history: [...s.history, prev], future: [] }));
  }
  function doUndo() {
    setApp((s: any) => {
  ...
  function startTimerIfNeeded() {
    if (!app.timer.running && app.timer.startedAt == null) {
      setApp((s: any) => ({
        ...s,
        timer: { startedAt: Date.now(), elapsed: 0, running: true },
      }));
    }
  }
  ...
  function setEntry(n: number) {
    ...
    if (app.pencilMode) {
      ...
      next[r][c] = n;
      const nextNotes = deserializeNotes(app.notes);
      nextNotes[r][c].clear();
      pushHistory(prev);
      setApp((s: any) => ({
        ...s,
  ...
  function checkCompletion(entries: Board) {
    ...
    setApp((s: any) => {
      const t = s.timer.elapsed;
      const diff = s.difficulty;
      const st = { ...s.stats } as any;
      st[diff] = {
        games: st[diff].games + 1,
        wins: st[diff].wins + 1,
        last: t,
        best: st[diff].best == null ? t : Math.min(st[diff].best, t),
      };
      return {
        ...s,
  ...
  function regenerate(difficulty: Diff = app.difficulty) {
    ...
    setApp((s: any) => {
      const st = { ...s.stats } as any;
      st[difficulty] = { ...st[difficulty], games: st[difficulty].games + 1 };
      return {
        ...s,
        difficulty,
        puzzle,
        ...
        history: [],
        future: [],
        timer: { startedAt: null, elapsed: 0, running: false },
        stats: st,
      };
    });
  }
  function handleRegenerate() {
    askConfirm(() => regenerate(app.difficulty));
  }
function handleChangeDifficulty(e: React.ChangeEvent<HTMLSelectElement>) {
  const next = e.target.value as Diff;   // <-- salviamo subito la scelta
  askConfirm(() => regenerate(next));    // <-- usiamo quel valore alla conferma
}
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLDivElement>) {
    ...
    if (e.key >= "1" && e.key <= "9") setEntry(parseInt(e.key, 10));
      ...
      setApp((s: any) => ({ ...s, pencilMode: !s.pencilMode }));
  }

  if (!app.puzzle || !app.solution) {
    return (
      <div className={cx("min-h-screen flex items-center justify-center", pastel.bg)}>
        <div className="text-center text-zen-forest animate-pulse">
          Sto generando un sudoku zen per te…
        </div>
      </div>
    );
  }
  ...
  return (
    <main
      className={cx("min-h-screen p-4 md:p-8 relative", pastel.bg)}
    ...
    >
      ...
      <div className="mx-auto max-w-5xl mb-4 md:mb-6 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h1 className="text-4xl sm:text-5xl font-bold text-zen-forest">
            Zenny Sudoku
          </h1>
          <img src="/chick.png" alt="Zenny" className="w-20 h-20 sm:w-24 sm:h-24" />
        </div>

        {/* mobile */}
        <div className="flex flex-col gap-2 sm:hidden">
          <button
            className="p-2 rounded-xl border border-zen-sage bg-white/70 hover:bg-zen-mint"
          ...
          >
            {app.musicMuted ? (
              <VolumeX className="w-5 h-5 text-zen-forest" />
            ) : (
              <Volume2 className="w-5 h-5 text-zen-forest" />
            )}
          </button>
          <button
            className="p-2 rounded-xl border border-zen-sage bg-white/70 hover:bg-zen-mint"
            ...
            title="Tutorial"
          >
            <BookOpen className="w-5 h-5 text-zen-forest" />
          </button>
        </div>
...
<Btn className="bg-white/70" onClick={() => { setTutorialPage(0); setTutorialOpen(true); }}>
  <BookOpen className="w-4 h-4" /> <span className="hidden sm:inline">Tutorial</span>
</Btn>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-5xl grid md:grid-cols-[1fr_380px] gap-6 items-start">
          ...
          <div className="mx-auto w-full max-w-[min(92vw,560px)]">
            ...
            <div className="mb-3 flex flex-wrap justify-center gap-3">
              <select
                ...
                className="h-10 px-3 rounded-xl border border-zen-sage bg-white text-zen-forest"
                title="Difficoltà"
              >
            ...
            {/* timer + seed */}
            <div className="mb-3 flex items-center justify-between text-zen-forest/90">
              <div className="flex items-center gap-2">
                <TimerReset className="w-4 h-4" />
                <span>{formatTime(app.timer.elapsed)}</span>
              </div>
              <div className="text-xs text-zen-forest/70">Seed: {app.seed}</div>
            </div>
        ...
        <div className="space-y-6">
          <Card>
            <h2 className="text-xl font-semibold text-zen-forest mb-2">
              Statistiche
            </h2>
      ...
      <Modal
        ...
        actions={
          <>
            ...
            <Btn variant="primary" onClick={doConfirm}>
              Sì, continua
            </Btn>
          </>
        }
      ...
      <Modal
        ...
        description={
          <div className="flex items-center gap-2 text-zen-forest">
            Tempo: {formatTime(app.timer.elapsed)}
          </div>
        }
        showIcon={false}
        actions={
      ...
      >
        <div className="mt-2 flex items-center gap-2">
          ...
          <span className="text-zen-forest">Il polletto è felice! </span>
        </div>
      </Modal>
...
function StatsCard({ stats }: { stats: ReturnType<typeof defaultStats> }) {
  ...
  return (
    <div className="space-y-3 text-zen-forest">
      <div className="grid grid-cols-3 text-sm font-medium text-zen-forest">
        <div></div>
        <div className="text-center">Giocate/Vinte</div>
        <div className="text-right">Migliore / Ultima</div>
      </div>
      {rows.map(({ k, label }) => (
        <div key={k} className="grid grid-cols-3 text-sm items-center">
          <div className="font-medium text-zen-forest">{label}</div>
          <div className="text-center">
            {stats[k].games} / {stats[k].wins}
          </div>
          <div className="text-right">
            {stats[k].best != null ? formatTime(stats[k].best) : "—"} /{" "}
            {stats[k].last != null ? formatTime(stats[k].last) : "—"}
          </div>
        </div>
      ))}
    </div>
